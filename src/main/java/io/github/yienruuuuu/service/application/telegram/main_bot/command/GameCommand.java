 ...
 */
@Component
public class GameCommand extends BaseCommand implements MainBotCommand {

    public GameCommand(UserService userService, LanguageService languageService, TelegramBotClient telegramBotClient, AnnouncementService announcementService) {
    ...
    public void execute(Update update, Bot mainBotEntity) {
        ...
        //傳送說明訊息
        String getPointDescription = super.getAnnouncementMessage(AnnouncementType.GAME_DESCRIPTION, user.getLanguage()).orElse("NOT_FOUND");
        telegramBotClient.send(SendDice.builder().chatId(chatId).build(), mainBotEntity);
